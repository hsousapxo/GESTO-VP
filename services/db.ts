...
const DB_VERSION = 3; // Incremented version to add contacts store
...
export const saveFlight = async (flight: FlightFormData): Promise<string> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const record = { ...flight, id: flight.id || crypto.randomUUID(), createdAt: flight.createdAt || new Date() };
...
// CONTACTS OPERATIONS
export const saveContact = async (contact: any): Promise<string> => {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const transaction = db.transaction(STORE_CONTACTS, 'readwrite');
        const store = transaction.objectStore(STORE_CONTACTS);
        const record = { ...contact, id: contact.id || crypto.randomUUID() };
        const request = store.put(record);
        request.onsuccess = () => resolve(record.id);
        request.onerror = () => reject(request.error);
    });
...
export const getContacts = async (category?: string): Promise<any[]> => {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const transaction = db.transaction(STORE_CONTACTS, 'readonly');
        const store = transaction.objectStore(STORE_CONTACTS);
        const request = category ? store.index('category').getAll(category) : store.getAll();
        request.onsuccess = () => resolve(request.result);
        request.onerror = () => reject(request.error);
    });
};